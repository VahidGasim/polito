...
public class Delivery {
    ...
    public int newCustomer(String name, String address, String phone, String email) throws DeliveryException {
        if(customers.values().stream().map(Customer::getEmail).findAny() == null) 
        	throw new DeliveryException("Duplicate customer");
    ...
    public List<String> listCustomers(){
        return customers.values().stream().sorted(Comparator.comparing(Customer::getName)).map(Customer::toString).collect(Collectors.toList());
    }
    ...
    public void newMenuItem(String description, double price, String category, int prepTime){
        items.put(description, new Item(description,price,category,prepTime));
    }
    ...
    public List<String> findItem(String search){
    	 Stream<Item> its=items.values().stream();
         if(search.length()>0){
             its = its.filter(it -> it.match(search));
         }
         return its.sorted(Comparator.comparing(Item::getCategory).thenComparing(Item::getDescription))
                 .map(Item::toString).collect(Collectors.toList());
    }
    ...
    public int newOrder(int customerId){
    	...
    	orders.put(++oid, new Order(oid, c));
    ...
    public int addItem(int orderId, String search, int qty) throws DeliveryException {
         ...
         int q = o.addItem(item.get(0),qty);
         return q;
    }
    ...
    public List<String> showOrder(int orderId) throws DeliveryException {
    	//if(!orders.containsKey(orderId)) throw new DeliveryException("Blet dagidaram...");
    	//Order o = orders.get(orderId);
    	//List<String> l = o.toStringList();
    	//System.out.println();
        return null;
    }
