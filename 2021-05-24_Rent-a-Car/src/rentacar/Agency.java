...
public class Agency {
	...
	private Map<Character, Integer> catnumlist = new HashMap<>();
	...
	int addVehicle(Vehicle vehicle) throws AgencyException {
		...
		catnumlist.put(vehicle.getCategory(), catnum++);
	...
	public Map<Integer, List<String>> getCarInfoForYears() {
		return vehicles.stream().sorted(Comparator.comparing(Vehicle::getYear).reversed())
				.collect(Collectors.groupingBy(Vehicle::getYear, 
									TreeMap::new, 
						Collectors.mapping(v -> String.format("%s:%s:%s", v.getManufacturer(), v.getModel(), v.getColor()), Collectors.toList())));
